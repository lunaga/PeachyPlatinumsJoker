...
interface Product {
    ...
    return (
        <Card
        className={`group bg-gray-900/50 backdrop-blur-sm border transition-all duration-300 hover:shadow-lg overflow-hidden h-full ${
        ...
        <div className="relative overflow-hidden">
            ...
            <Badge className={`${categoryBadge.color} border-0 shadow-lg font-bold text-xs`}>{categoryBadge.label}</Badge>
            </div>
            <div className="absolute top-2 right-2">
            <div
                className={`backdrop-blur-sm rounded-full p-1.5 ${
                product.category === "bundles" ? "bg-orange-500/70" : "bg-black/70"
                }`}
            >
                <IconComponent className={`h-3 w-3 ${product.category === "bundles" ? "text-white" : "text-yellow-400"}`} />
            </div>
            </div>

            {/* Bundle Special Indicator */}
            {product.category === "bundles" && (
            <div className="absolute top-0 right-0 bg-gradient-to-l from-red-500 to-orange-500 text-white text-xs font-bold px-2 py-1 rounded-bl-lg">
                OFFER
            </div>
            )}
        </div>
        ...
        <CardContent className="p-3 flex-1">
            <h3 className="font-bold text-sm text-purple-100 mb-2 line-clamp-2 group-hover:text-purple-50 transition-colors duration-300 leading-tight">
            {product.name}
            </h3>
            <p className="text-purple-300 text-xs mb-3 line-clamp-2 leading-tight">{product.description}</p>
            ...
            {product.category === "bundles" && (
                <span className="text-xs text-orange-300 font-semibold bg-orange-500/20 px-2 py-0.5 rounded-full">
                BUNDLE
                </span>
            )}
            </div>
        ...
        <CardFooter className="p-3 pt-0">
            ...
            className={`w-full font-semibold py-2 px-3 rounded-lg transition-all duration-300 shadow-md text-sm ${
                product.category === "bundles"
            ...
            {product.category === "bundles" ? "Add Bundle" : "Add to Cart"}
            </Button>
        </CardFooter>
        </Card>
    )
